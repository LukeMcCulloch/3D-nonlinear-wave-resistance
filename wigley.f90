...
PROGRAM wigley
  ...
  real(wp), dimension(:), allocatable :: ldx
...
!!*******************************Improvising an x shift!**************************
  ...
  ! since the dx step is constant in this version, 
  ! we could have made this simpler:
  ! at least when it does vary,
  ! it might be computationaly efficient
  ! to remove computation of the dx 
  ! outside the loop - 
  ! also, it will be in python... anyway...
  do i=1,nfsl
     ldx(i)=dx
  end do
...
!!
  ...
  CALL savesphere( outputfile, 'A Wigley discretization', (/0,1/), &
                   npanels, nfspanels, npoints, deltax, panels, points, nfsl, nfst,ldx )
