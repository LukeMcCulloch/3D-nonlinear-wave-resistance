...
def polymake(surf1, FSheight, panelShift, nhl, nhg, nhf, Compute_Curvatures,L,B,T):
    ...
    Hull_Constant_Dx = L/nhl
    print 'the hull is assumed to be {} in length'.format(L)
    print 'with {} panels'.format(nhl)
    print 'the hull constant step Dx will be {}'.format(Hull_Constant_Dx)

    for j in range(nhg + nhf +1 ): #transverse direction including free surface panel.
        
        LongiHullpt=0. # each we increment in u start at the hull beggining again
    
        #u=float(j)/float(nhg + nhf)
        u=float(j)/float(nhg)
        for i in range(nhl+1):                              #longitudinal direction - that will effect z offset...
            #v=float(i)/float(nhl) ##------------->longi calc:
            v, ThisQ, found, ThisCount = surf1.findPtloc(LongiHullpt, u, 1, 0)
            
            if j <= nhg:
                LongiHullpt += Hull_Constant_Dx
...
def poly_prep(L,B,T,surf1,Compute_Curvatures=False,
    ...
    points, panels,Bfs,yfs,npoints,npanels,nfspanels,ntransompanels,ntransomt,ntransoml,tnpt =\
            polymake(surf1, FSH, panelShift, nhl, nhg, nhf, Compute_Curvatures, L,B,T)
